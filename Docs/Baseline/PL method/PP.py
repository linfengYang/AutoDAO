import numpy as np
def solve_rolling_uc_PP(units_info, load):
    ...
    for unit in units_info:
        ...
        if Pmax == 0:
            ...
            HR = (unit['a_i'] + unit['b_i'] * Pmax + unit['c_i'] * Pmax ** 2) / Pmax
        HR_list.append(HR)

    # Create priority list (higher Pmax first, lower HR if tie)
    priority_indices = sorted(range(n_units),
                              key=lambda i: (-units_info[i]['p_max_i'], HR_list[i]))

#########################################################

    sorted_indices = priority_indices
